...
if __name__ == '__main__':
    ...
    # --- eXplainable AI (XAI) Section: SHAP Analysis ---
    ...
    for idx, hist in enumerate(all_histories):
        if 'val_f1' in hist and len(hist['val_f1']) > 0:
            ...
            if avg_f1 > best_global_f1:
                ...
                best_global_idx = idx
    xai_model_path = all_model_paths[best_global_idx]
...
# -------- Generate feature names--------------------------------------------------------------------------------------------------------
    ...
    feature_names = []
    # Spatial features: 81 positions x 16 times x 5 bands
    for pos in range(81):
        r, c = divmod(pos, 9)
        for t in range(16):
            for b in range(5):
                feature_names.append(f"Spatial_r{r}_c{c}_t{t}_band{bands[b]}")
    # Temporal features: 310 channels x 16 times
    for ch in range(310):
        for t in range(16):
            feature_names.append(f"Temporal_ch{ch}_t{t}")
    # Eye features: 33 base features x 16 times
    feature_ranges = {
