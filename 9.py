...
# 7. video-to-emotion-to-integer mapping from the table
video_to_emotion = {
...
for i in range(1, 21):
    ...
    for key in sorted_de_lds:
        ...
        for band in band_names:
            ...
            filtered_data = filter_frequency_bands(original_data, [band_index])  #(n_samples, 1, 62)
...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 64
    NUM_EPOCHS = 100
    skf_outer = StratifiedKFold(n_splits=outer_kfolds, shuffle=True, random_state=42)
    all_inner_histories = []
    all_best_inner_paths = []
    for outer_fold, (train_outer_idx, val_outer_idx) in enumerate(skf_outer.split(all_spatial_grids.cpu(), all_labels.cpu())):
        print(f"\n--- Starting Outer Fold {outer_fold + 1}/{outer_kfolds} ---")

        s_test = all_spatial_grids[val_outer_idx].to(device)
        t_test = all_temporal_raw[val_outer_idx].to(device)
        e_test = all_eye_raw[val_outer_idx].to(device)
        l_test = all_labels[val_outer_idx].to(device)
        test_data = (s_test, t_test, e_test, l_test)

        train_val_spatial = all_spatial_grids[train_outer_idx]
        train_val_temporal = all_temporal_raw[train_outer_idx]
        train_val_eye = all_eye_raw[train_outer_idx]
        train_val_labels = all_labels[train_outer_idx]
        ...
        for fold_k, (train_k_idx, val_k_idx) in enumerate(skf.split(train_val_spatial, train_val_labels)):
            ...
            model = FeatureEncoder(feature_dim=FEATURE_DIM).to(device)          
            best_model_file, history = train_and_evaluate(model, train_loader, val_loader, test_data, num_epochs=NUM_EPOCHS)

            unique_model_path = f'best_model_outer{outer_fold+1}_inner{fold_k+1}.pth'
            os.rename(best_model_file, unique_model_path)
            best_model_paths.append(unique_model_path)
            inner_histories.append(history)
            all_inner_histories.append(history)
            print(f" Inner K-Fold {fold_k + 1} Best Validation Loss Reached. Model Saved.")
            plot_metrics(history, outer_fold + 1)
            
        best_inner_idx = 0
        best_avg_f1 = -1.0
        for idx, hist in enumerate(inner_histories):
            if 'val_f1' in hist and len(hist['val_f1']) > 0:
                avg_f1 = np.mean(hist['val_f1'])
                if avg_f1 > best_avg_f1:
                    best_avg_f1 = avg_f1
                    best_inner_idx = idx
        selected_model_path = best_model_paths[best_inner_idx]

        all_best_inner_paths.append(selected_model_path)
        print(f" xAI Selected Best Inner Model: {selected_model_path}")
        ...
        print(f"Outer Fold {outer_fold + 1}: Final ENSEMBLE Test Accuracy = {final_test_accuracy:.4f}")
        ...
        print(f"Outer Fold {outer_fold + 1} Classification Report:\n{classification_report(y_true, y_pred)}")
        ...
        plt.title(f'Outer Fold {outer_fold + 1} Confusion Matrix (Numerics)')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.savefig(f'cm_fold_{outer_fold + 1}.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        plt.title(f'Outer Fold {outer_fold + 1} Confusion Matrix (Percentages)')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.savefig(f'cm_fold_percent_{outer_fold + 1}.png', dpi=300, bbox_inches='tight')
        plt.show()
        # ROC-AUC Score (macro-averaged)
        fold_auc = roc_auc_score(y_true, y_proba, multi_class='ovr', average='macro')
        fold_auc_scores.append(fold_auc)
        print(f"Outer Fold {outer_fold + 1} ROC-AUC Score (macro): {fold_auc:.4f}")
    ...
    print(f"\n--- Final K-Fold CV Result ---")
    print(f"Average K-Fold CV Test Accuracy: {mean_accuracy:.4f}")
   
    mean_auc = sum(fold_auc_scores) / len(fold_auc_scores)
    print(f"Average K-Fold CV ROC-AUC Score (macro): {mean_auc:.4f}")
    ...
    global_best_idx = 0
    global_best_avg_f1 = -1.0
    for idx, (hist, path) in enumerate(zip(all_inner_histories, all_best_inner_paths)):
        if 'val_f1' in hist and len(hist['val_f1']) > 0:
            avg_f1 = np.mean(hist['val_f1'])
            if avg_f1 > global_best_avg_f1:
                global_best_avg_f1 = avg_f1
                global_best_idx = idx
    xai_model_path = all_best_inner_paths[global_best_idx]
    print(f" xAI Selected Global Best Model for XAI: {xai_model_path}")
    if not os.path.exists(xai_model_path):
        raise FileNotFoundError(f"Trained model not found at {xai_model_path}. Run training first.")
   
    xai_model = torch.load(xai_model_path, weights_only=False)
    xai_model.to(device)
    xai_model.eval()
    num_background = 100
    background_spatial = all_spatial_grids[:num_background].cpu()
    background_temporal = all_temporal_raw[:num_background].cpu()
    background_eye = all_eye_raw[:num_background].cpu()
    background_data_list = []
    for i in range(num_background):
        s_flat = background_spatial[i].flatten().numpy()
        t_flat = background_temporal[i].flatten().numpy()
        e_flat = background_eye[i].flatten().numpy()
        combined = np.concatenate([s_flat, t_flat, e_flat])
        background_data_list.append(combined)
    background = np.array(background_data_list)
    num_instances = 10
    instance_indices = range(100, 100 + num_instances)
    instance_spatial = all_spatial_grids[instance_indices].cpu()
    instance_temporal = all_temporal_raw[instance_indices].cpu()
    instance_eye = all_eye_raw[instance_indices].cpu()
    instance_labels = all_labels[instance_indices].cpu().numpy()
    spatial_size = 9 * 9 * 80 # 6480
    temporal_size = 16 * 310 # 4960
    eye_size = 16 * 33 # 528
    total_features = spatial_size + temporal_size + eye_size # 11968
# -------- Generate feature names--------------------------------------------------------------------------------------------------------
    ...
    for pos in range(81):
        ...
        for t in range(16):
            for b in range(5):
                feature_names.append(f"Spatial_r{r}_c{c}_t{t}_band{bands[b]}")
    for ch in range(310):
        for t in range(16):
            feature_names.append(f"Temporal_ch{ch}_t{t}")
    feature_ranges = {
        'pupil_diameters': (0, 11),
        'fixation_duration': (12, 13),
        'dispersion': (14, 17),
        'saccade_duration': (18, 19),
        'saccade_amplitude': (20, 21),
        'blink_duration': (22, 23),
        'event_statistics': (24, 32)
    }
    eye_base_names = []
    for key, (start, end) in feature_ranges.items():
        for idx in range(start, end + 1):
            eye_base_names.append(f"{key}_{idx - start}")
    for base_name in eye_base_names:
        for t in range(16):
            feature_names.append(f"Eye_{base_name}_t{t}")
    assert len(feature_names) == total_features, f"Feature names length {len(feature_names)} != {total_features}"
# ---------------------------------------------------------------------------------------------------------------------------------------
    def model_predict(data):
        N = data.shape[0]
        assert data.shape[1] == total_features
        spatial = data[:, :spatial_size].reshape(N, 9, 9, 80)
        temporal = data[:, spatial_size:spatial_size + temporal_size].reshape(N, 16, 310)
        eye = data[:, -eye_size:].reshape(N, 16, 33)
        spatial_t = torch.from_numpy(spatial).float().to(device)
        temporal_t = torch.from_numpy(temporal).float().to(device)
        eye_t = torch.from_numpy(eye).float().to(device)
        with torch.no_grad():
            outputs = xai_model(spatial_t, temporal_t, eye_t)
            probs = F.softmax(outputs, dim=1).cpu().numpy()
        return probs
        
    def logit_predict(data):
        N = data.shape[0]
        assert data.shape[1] == total_features
        spatial = data[:, :spatial_size].reshape(N, 9, 9, 80)
        temporal = data[:, spatial_size:spatial_size + temporal_size].reshape(N, 16, 310)
    ...
    for i in instance_indices:
        s_flat = all_spatial_grids[i].flatten().cpu().numpy()
        t_flat = all_temporal_raw[i].flatten().cpu().numpy()
        e_flat = all_eye_raw[i].flatten().numpy()
        combined = np.concatenate([s_flat, t_flat, e_flat])
        instance_data_list.append(combined)
    instance_data = np.array(instance_data_list)
    instance_probs = model_predict(instance_data)
    instance_predictions = np.argmax(instance_probs, axis=1)
    print(f"Predictions for instances: {instance_predictions}")
    print(f"True labels: {instance_labels}")
    
    class FlattenedModel(nn.Module):
        def __init__(self, original_model, spatial_size, temporal_size, eye_size):
            super().__init__()
        ...
        def forward(self, flat_input):
            B = flat_input.size(0)
            spatial_flat = flat_input[:, :self.spatial_size].view(B, 9, 9, 80)
            temporal_flat = flat_input[:, self.spatial_size:self.spatial_size + self.temporal_size].view(B, 16, 310)
            eye_flat = flat_input[:, -self.eye_size:].view(B, 16, 33)
            return self.model(spatial_flat, temporal_flat, eye_flat)
    xai_model_wrapper = FlattenedModel(xai_model, spatial_size, temporal_size, eye_size).to(device)

    background_torch = torch.from_numpy(background).float().to(device)
    test_instance_torch = torch.from_numpy(instance_data[0]).float().unsqueeze(0).to(device)
    
    print("Computing SHAP explanations with GradientExplainer...")
   
    shap_explainer = shap.GradientExplainer(xai_model_wrapper, background_torch)
    shap_values = shap_explainer.shap_values(test_instance_torch, nsamples=100)
    
    predicted_class = instance_predictions[0]
    shap_values_pred = shap_values[0, :, predicted_class]
    background_logits = logit_predict(background)
    base_values = np.mean(background_logits[:, predicted_class])

    shap_values_exp = Explanation(
        values=shap_values_pred,
        base_values=base_values,
        data=instance_data[0],
        feature_names=feature_names
    )
    shap.plots.waterfall(shap_values_exp, max_display=100, show=False)
    plt.title(f'SHAP Waterfall Plot for Predicted Class {predicted_class}')
    plt.savefig(f'shap_waterfall_class_{predicted_class}.png', dpi=300, bbox_inches='tight')
    plt.show()

#  12. Model Summary
model_path = 'best_model_outer1_inner1.pth'
